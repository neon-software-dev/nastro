...
std::expected<HDU::Type, Error> GetHDUType(const Header& header)
{
    ...
    {
        // TODO! Validate that false is logical T
    ...
    return HDU::Type::Image; // TODO!
}

std::expected<uintmax_t, Error> GetHDUDataByteSize_Image(const Header& header)
{
    const auto bitpix = header.GetFirstKeywordRecord(KEYWORD_NAME_BITPIX);
    if (!bitpix) { return std::unexpected(Error::Msg(ErrorType::Validation, "GetHDUDataByteSize_Image: Header is missing BITPIX keyword")); }

    const auto bitpixValue = (*bitpix)->GetKeywordValueAsInteger();
    if (!bitpixValue) { return std::unexpected(Error::Msg(ErrorType::Parse, "GetHDUDataByteSize_Image: Failed to parse BITPIX value to integer")); }

    const auto naxis = header.GetFirstKeywordRecord(KEYWORD_NAME_NAXIS);
    if (!naxis) { return std::unexpected(Error::Msg(ErrorType::Validation, "GetHDUDataByteSize_Image: Header is missing NAXIS keyword")); }

    const auto naxisValue = (*naxis)->GetKeywordValueAsInteger();
    if (!naxisValue) { return std::unexpected(Error::Msg(ErrorType::Parse, "GetHDUDataByteSize_Image: Failed to parse NAXIS value to integer")); }
    ...
    for (intmax_t n = 0; n < *naxisValue; ++n)
    {
        ...
        const auto naxisn = header.GetFirstKeywordRecord(keywordName);
        if (!naxisn) { return std::unexpected(Error::Msg(ErrorType::Validation, "GetHDUDataByteSize_Image: Header is missing {} keyword", keywordName)); }

        const auto naxisnValue = (*naxisn)->GetKeywordValueAsInteger();
...
std::expected<uintmax_t, Error> GetHDUDataByteSize(const HDU::Type& type, const Header& header)
{
    ...
    switch (type)
    {
        ...
        case HDU::Type::Empty: dataBlockCount = 0; break;
    }
