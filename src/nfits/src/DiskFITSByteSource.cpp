...
std::expected<ByteSize, Error> DiskFITSByteSource::GetByteSize() const
{
    ...
    if (ec)
    {
        return std::unexpected(Error::Msg(ErrorType::General, "DiskFITSByteSource::GetByteSize: Call to file_size() failed"));
    }
...
Result DiskFITSByteSource::Resize(const ByteSize& byteSize)
{
    ...
    if (ec)
    {
        return Result::Fail(ErrorType::General, "DiskFITSByteSource::Resize: Call to resize_file() failed");
    }
...
Result DiskFITSByteSource::ReadBytes(std::span<std::byte> dst, const ByteOffset& byteOffset, const ByteSize& byteSize)
{
    if (!m_stream.is_open())
    {
        return Result::Fail(ErrorType::General, "DiskFITSByteSource::ReadBytes: File stream isn't open");
    }

    if (dst.size() < byteSize.value)
    {
        return Result::Fail(ErrorType::General, "DiskFITSByteSource::ReadBytes: dst size is too small for requested read");
    }
    ...
    if (!m_stream.good())
    {
        return Result::Fail(ErrorType::General, "DiskFITSByteSource::ReadBytes: Failed to seek to byte offset");
    }
    ...
    if (!m_stream.good())
    {
        return Result::Fail(ErrorType::General, "DiskFITSByteSource::ReadBytes: Failed to read bytes from the stream");
    }
...
Result DiskFITSByteSource::WriteBytes(std::span<const std::byte> src, const ByteOffset& byteOffset, const ByteSize& byteSize, bool flush)
{
    if (!m_stream.is_open())
    {
        return Result::Fail(ErrorType::General, "DiskFITSByteSource::WriteBytes: File stream isn't open");
    }

    if (src.size() < byteSize.value)
    {
        return Result::Fail(ErrorType::General, "DiskFITSByteSource::WriteBytes: src size is too small for requested write");
    }
    ...
    if (!m_stream.good())
    {
        return Result::Fail(ErrorType::General, "DiskFITSByteSource::WriteBytes: Failed to seek to byte offset");
    }
    ...
    if (!m_stream.good())
    {
        return Result::Fail(ErrorType::General, "DiskFITSByteSource::WriteBytes: Failed to read bytes from the stream");
    }
...
Result DiskFITSByteSource::Flush()
{
    if (!m_stream.is_open())
    {
        return Result::Fail(ErrorType::General, "DiskFITSByteSource::Flush: File stream isn't open");
    }
...
Result DiskFITSByteSource::OpenStream(bool createIfNotExists)
{
    ...
    if (!std::filesystem::exists(m_filePath))
    {
        ...
        {
            return Result::Fail(ErrorType::General, "DiskFITSByteSource: File doesn't exist");
        }
        ...
        if (!createStream.is_open())
        {
            ...
            return Result::Fail(ErrorType::General, "DiskFITSByteSource: Failed to open/create the file");
        }
    ...
    if (!m_stream.is_open())
    {
        ...
        return Result::Fail(ErrorType::General, "DiskFITSByteSource: Failed to open the file");
    }
